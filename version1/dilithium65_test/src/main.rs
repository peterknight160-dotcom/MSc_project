...
fn check_fips204(message: &[u8], iterations: usize) -> Option<bool> {
    ...
    for _ in 0..iterations {
        let _ = MlDsaKeyPair::generate(ML_DSA_65).ok();
    }
    ...
    for _ in 0..iterations {
          let _ = MlDsaKeyPair::verify(    kp.public_key(), &sig,    message, b"",    ML_DSA_65);
    }
    ...
    let v = MlDsaKeyPair::verify(kp.public_key(), &sig, message, b"", ML_DSA_65); // Use the public to verify message signature
    Some(v)
}
