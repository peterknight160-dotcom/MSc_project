use fips204::ml_dsa_65; // Could also be ml_dsa_65 or ml_dsa_65.
...
fn check_fips204(message: &[u8], iterations: usize) -> Option<bool> {
    ...
    let Some((pk1, sk)) = ml_dsa_65::try_keygen().ok() else {
        panic!("At line 7")
    ...
    for _ in 0..iterations {
        let _: Option<[u8; 3309]> = sk.try_sign(&message, &[]).ok(); // Use the secret key to generate a message signature
    ...
    let pk2 = ml_dsa_65::PublicKey::try_from_bytes(pk_recv).ok();
