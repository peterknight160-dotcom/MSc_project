use dilithium::{MlDsaKeyPair, ML_DSA_87};
...
fn check_fips204(message: &[u8], iterations: usize) -> Option<bool> {
    ...
    for _ in 0..iterations {
        let _ = MlDsaKeyPair::generate(ML_DSA_87).ok();
    }
    ...
    for _ in 0..iterations {
          let _ = MlDsaKeyPair::verify(    kp.public_key(), &sig,    message, b"",    ML_DSA_87);
    }
    ...
    let v = MlDsaKeyPair::verify(kp.public_key(), &sig, message, b"", ML_DSA_87); // Use the public to verify message signature
    Some(v)
}
