...
fn check_fips203(nloops: u32) {
    ...
    let mut key_time_hash: BTreeMap<u128, u32> = BTreeMap::new();
     ...
     // Pre -run
        ...
        let (ct, _ss_sender) = kyber::safe_encaps(ML_KEM_512, kp.public_key()).unwrap();
        let _ss_receiver = kp.decaps(&ct).unwrap();
    ...
    let stats512 = stats_from_btree(&key_time_hash, "Lattice_Kyber_512 Keys Swap");
    // Get mean + 2 std devs
    let twosigma = stats512.mean * 1.25;

    let _ = draw_histogram_from_btree(&key_time_hash, "Lattice_Kyber_512_Keys_Swap", twosigma);
    ...
    for _ in 0..nloops {
        let start_key = Instant::now();
        let kp = MlKemKeyPair::generate(ML_KEM_768).unwrap();
        let (ct, _ss_sender) = kyber::safe_encaps(ML_KEM_768, kp.public_key()).unwrap();
        let _ss_receiver = kp.decaps(&ct).unwrap();
    ...
    let stats768 = stats_from_btree(&key_time_hash, "Lattice_Kyber_768 Keys Swap");
    // Get mean + 2 std devs
    let twosigma = stats768.mean * 1.25;

    let _ = draw_histogram_from_btree(&key_time_hash, "Lattice_Kyber_768_Keys_Swap", twosigma);
    ...
    for _ in 0..nloops {
        ...
        let (ct, _ss_sender) = kyber::safe_encaps(ML_KEM_1024, kp.public_key()).unwrap();
        let _ss_receiver = kp.decaps(&ct).unwrap();
    ...
    let stats1024 = stats_from_btree(&key_time_hash, "Lattice_Kyber_1024 Keys Swap");
    // Get mean + 2 std devs
    let twosigma = stats1024.mean * 1.25;

    let _ = draw_histogram_from_btree(&key_time_hash, "Lattice_Kyber_1024_Keys_Swap", twosigma);
