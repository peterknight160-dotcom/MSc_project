...
use useful_stats::*;
fn check_fips204(message: & 'static[u8], nkeys: u32, nloops: u32) -> Option<bool> {
    ...
    for _ in 0..nkeys {
        ...
        *key_time_hash.entry(key_time).or_insert(0) += 1;
        ...
        for _ in 0..nloops {
            ...
            let tx1 = tx.clone() ;
            let m1_thread = Arc::clone(&m1);
              let m2_thread = Arc::clone(&m2);
            thread::spawn(move || {
                let s1 = kp1.sign(&m1_thread, &[]).unwrap();
            let _ = tx1.send(s1) ;});
            let tx2 = tx.clone() ;
            thread::spawn(move || {
                let s2 = kp2.sign(&m2_thread, &[]).unwrap();
            let _ = tx2.send(s2);});
...
let message: &'static [u8] = Box::leak(
    std::fs::read("json.txt").expect("Unable to read file").into_boxed_slice()
);
