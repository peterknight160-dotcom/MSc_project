...
async fn handle_connection(
    ...
    //
    let receiver_public= vec![1u8; 32]; // Placeholder for the receiver's public key, replace with actual key retrieval logic
  
    let s = text_key_to_send(&signature_keys, &receiver_public)?;
   ...
   let sender_public = get_text_keys(&signature_keys, &mut socket).await?;
   // if not error, then we have the keys, now we can decapsulate to get the shared secret


    let ss_receiver = vec![2u8; 32]; // Placeholder for the shared secret, replace with actual decapsulation logic
