...
async fn main() -> std::io::Result<()> {
    ...
    let pub_key = get_text_keys(&signature_keys, &mut stream).await.unwrap();
    ...
    let  sender_public = vec![0u8; 32]; // Placeholder for sender's public key
    
    let s = text_key_to_send(&signature_keys, &sender_public)?;
    stream.write_u32(s.len() as u32).await?;
    stream.write_all(&s).await?;


    let shared_secret = vec![0u8; 32]; // Placeholder for shared secret
    ...
    //println!("Shared Secret is {:?}", base65::base64_from_bytes(shared_secret.as_bytes()).unwrap());
        ...
        let stats_auth = stats_from_btree(&auth_time_hash, "No Authentication");
    // Get mean + 2 std devs
    let twosigma = stats_auth.mean + 2.0 * stats_auth.std_dev;

    let _ = draw_histogram_from_btree(&auth_time_hash, "No_Authentication", twosigma);
